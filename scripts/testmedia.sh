...
# ---------------------------------------------------------------------------------------------
# Provenance. Register row KP-FIXTURE-PIN.
#
...
#
# This manifest does not GATE anything, on purpose: a checksum gate would turn every legitimate
# ffmpeg upgrade into a red build. It records what made the clips so the next difference is a diff
...
MANIFEST=MANIFEST.txt
{
    ...
    echo "# Nothing verifies this file. It exists so that 'why did that row behave differently'"
    echo "# has an answer that is a diff rather than a guess."
    echo
...
echo
echo "wrote testmedia/$MANIFEST: $(($(wc -l < "$MANIFEST") - 6)) fixtures, generator $(ffmpeg -version 2>/dev/null | head -1 | cut -d' ' -f3)"
