...
macOS arm64 remains the only T3 candidate. The named iOS simulator, the desktop JVM and Linux are
candidates above T1, but each only for the narrower slice described above; no label here grants a tier. No platform here
...
| `kiteplayer-subtitles` | the text subtitle parsers in pure common Kotlin: SubRip, WebVTT, and SubStation Alpha with its dialogue-grade override subset. Cues from these ARE timed, laid out and drawn: the engine schedules them and the three rasterizers in `kiteplayer-output` turn them into overlay images | every target it declares |
...
| `kiteplayer-network` | **not published.** The Ktor half of the custom AVIO bridge plus a zero-dependency DASH manifest parser, so http and https play with the operating system supplying TLS. Optional by construction: an app that plays local files only never depends on it and ships no Ktor. It stays out of the release until the streaming work in the plan settles its API | every target it declares |
| `kiteplayer-libass` | **not published.** Typesetting-grade ASS rendering through libass, built for seven target families and device-proven. It has no call sites outside its own tests yet, and publishing an artifact whose whole purpose needs a native chain the consumer cannot obtain would be a promise this repository cannot keep | seven native target families plus an Android JNI adapter, and only where the ass chain is present at build time |
