...
abstract class BuildLibassHostJniTask @Inject constructor(
    ...
    fun build() {
        ...
        val command: List<String> = when (triple) {
            ...
            "linux-x64", "linux-arm64" -> {
                ...
                val jniMd = writeJniMd("linux", "__attribute__((visibility(\"default\")))", "", "long long")
            ...
            "windows-x64" -> {
                ...
                val jniMd = writeJniMd("win32", "__declspec(dllexport)", "__stdcall", "long long")
                listOf(
                    konan.clang, "-target", konan.triple, "--sysroot=${konan.sysroot}",
    ...
    /** Under the task's temporary directory, never under [outputDir]: that directory becomes jar resources whole. */
    private fun writeJniMd(platform: String, export: String, call: String, jlong: String): File {
        val dir = temporaryDir.resolve("jni-md/$platform").also { it.mkdirs() }
