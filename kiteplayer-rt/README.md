...
| The supervised device run | 1, one machine, one debug binary | The worst callback body against half the device period, with the collector running thousands of times, on a named device. Its negative control fails. | Release-mode qualification, which is B10's, and any platform other than this one. |
