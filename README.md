...
| Windows x64 (mingw) | T1, link-proven | FFmpeg n8.0 cross-builds, the engine and the FFmpeg backend compile, and the test executable links as a PE32+ binary. Nothing has been RUN: there is no Windows machine here, and that run is an owner rider like the physical iPhone one. |
