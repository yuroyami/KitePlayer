...
/** Applies each low transient once, whether first seen through lookahead or normal delivery. */
    ...
    fun apply(frame: SpectrumFrame, upcoming: UpcomingAudioEvent?, scale: Float) {
        ...
        if (delivery != null) {
            for (index in 0 until delivery.size) {
                ...
                if (event.detection.kind != AudioEventKind.LowTransient) continue
        ...
        if (event.generation != generation || event.analysisRevision != revision ||
            event.detection.kind != AudioEventKind.LowTransient || event.detection.strength <= 0f ||
