...
import io.github.yuroyami.kiteplayer.audioviz.AudioEventSource
import io.github.yuroyami.kiteplayer.audioviz.SpectrumFrame

/**
 * One scene decision per delivered batch. This gate does not recognize musical structure. It
 * remembers the last sequence of each source, because sequences are comparable only within one.
 */
    ...
    fun read(frame: SpectrumFrame): AudioEvent? {
        ...
        for (index in 0 until delivery.size) {
            ...
            if (event.generation != frame.generation || event.analysisRevision != frame.analysisRevision ||
                event.sequence <= sequences[source]) continue
