...
private class AndroidGpuCompletionTracker(
    ...
    fun unbind(binding: Long?, owner: Any): Boolean {
        ...
        batches.holdUntilNextProof(CommittedDraw(owner, null))
        requestProofFrame()
    ...
    fun record(binding: Long?, owner: Any, frame: KiteVideoFrame?): Boolean {
        ...
        if (drawingTimeMillis <= 0L) {
            batches.holdUntilNextProof(CommittedDraw(owner, frame))
            view.postInvalidateOnAnimation()
            return true
        }
        batches.record(drawingTimeMillis, CommittedDraw(owner, frame))
        return true
    }
    ...
    override fun onFrameMetricsAvailable(
        ...
        completed.forEach { draw -> state.frameCommitted(draw.owner, draw.frame) }
