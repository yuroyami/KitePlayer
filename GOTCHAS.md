...
- **A Gradle compile task with no sources reports BUILD SUCCESSFUL.** It prints `NO-SOURCE` and
  ...
  passed with zero errors because that module has no shared test sources at all, only `jvmTest`
...
- **`runBlocking` does not exist on wasm**, because there is no thread to block. A shared test
  written with it will not compile for the web, and this is what kept `:kiteplayer-ffmpeg` off
  wasmJs until 2026-08-30. The fix is `runTest`, not a source-set exile: exile is right only for
  a test that genuinely needs something the web lacks, like a real clip on a real filesystem.
  Narrowing a shared test's source set REMOVES it from every target that no longer sees it, which
  is silent. Moving both files out of `commonTest` here would have dropped 32 tests from the
  Android host run, and nothing would have gone red to say so. Count tests per target before and
  after any source-set move.
